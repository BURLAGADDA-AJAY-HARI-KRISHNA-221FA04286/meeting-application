...
@router.post("/meetings/{meeting_id}/export-linear")
async def export_to_linear(
    meeting_id: int,
    payload: LinearExportRequest = None,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """Create Linear issues from meeting tasks."""
    if payload is None:
        payload = LinearExportRequest()
    ...
    if not current_user.linear_access_token:
        raise HTTPException(
            status_code=401,
            detail="Linear account not connected. Please connect from settings.",
        )

    team_id = payload.team_id or settings.linear_team_id
    if not team_id:
        raise HTTPException(status_code=400, detail="Linear team ID not provided.")
    ...
    linear_service = LinearService(
        ...
        redirect_uri=settings.linear_redirect_uri or "",
    ...
    for task in tasks:
        ...
        try:
            issue_url = await linear_service.create_issue(
                access_token=current_user.linear_access_token,
                title=f"[Meeting Task] {task.title}",
            ...
            successful += 1
            logger.info("Created Linear issue for task %d", task.id)
        except Exception as exc:
            created_issues.append({"task_id": task.id, "error": str(exc)})
            logger.error("Linear API error for task %d: %s", task.id, exc)
